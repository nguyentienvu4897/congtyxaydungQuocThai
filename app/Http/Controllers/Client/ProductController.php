...
class ProductController extends Controller
{
    ...
    {
        $data['list'] = Product::where(['status'=>1 , 'cate_slug'=>$slug])->orderBy('id','DESC')->select('id','category','name','discount','price','images','slug','cate_slug','type_slug','description', 'content')
        ->paginate(12);
        $data['title'] = "Tất cả mẫu thiết kế";
        $data['content'] = 'none';
    ...
    public function allListCate($danhmuc)
    {
        ...
        ->orderBy('id','DESC')
        ->select('id','category','name','discount','price','images','slug','cate_slug','type_slug','description', 'content')
        ->paginate(12);
    ...
    public function allListType($danhmuc,$loaidanhmuc){
        ...
        ->paginate(5);
        $data['type'] = TypeProduct::where('slug',$loaidanhmuc)->first(['id','name','slug','cate_id','content']);
        $cate_id = $data['type']->cate_id;
        $data['category'] = Category::where('id', $cate_id)->first(['name', 'slug']);
    ...
    public function detail_product($cate,$type,$id)
    {
        ...
        ])->where('slug',$id)->first(['id','name','images','type_cate','category','sku','discount','price','content','size','description','slug','preserve','cate_slug','type_slug']);
        $data['news'] = Blog::where('status',1)->limit(8)->get(['id','title','image','description','created_at','slug']);
        $data['productlq'] = Product::where('category',$data['product']->category)->take(10)->get(['id','name','images','discount','price','slug','cate_slug','type_slug','description']);
        $viewoldpro = session()->get('viewoldpro', []);

        if(isset($viewoldpro[$id])) {
